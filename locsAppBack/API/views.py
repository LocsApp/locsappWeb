...
class FacebookRegister(APIView):
	...
	def post(self, request):
		...
		if "facebook_token" in request.data and "username" in request.data:
			...
			# We verify an user have the same email
			if "email" not in profile and Account.object.filter(profile["email"]):
		...
		if ("living_address" in request.data):
			if (isinstance(request.data["living_address"], list)):
				if (len(request.data["living_address"]) == 2):
					...
					return Response(
						{
							"Error": "The key 'living_address' must have two slots, the first for the alias and the second for the address"},
						status=401)
			else:
				return Response(
