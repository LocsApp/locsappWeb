...
def searchArticles(request):
	...
	if ("_order" in body):
		...
		for fields_order in body["_order"]:
			if (not isinstance(fields_order, type({})) or len(
					fields_order.keys()) != 2 or not "order" in fields_order or not "field_name" in fields_order or not isinstance(
					fields_order["order"], str) or not isinstance(fields_order["field_name"],
			                                                      str)):
				return JsonResponse(
...
def demandsMain(request):
	...
	if request.method == "POST":
		return APIrequests.POST(
			...
			verifyIfDemandAlreadyIssued)
	elif request.method == "GET":
		return APIrequests.GET(
			'article_demands',
			special_field={"id_target": request.user.pk, "visible": True, "status": "pending"})
	else:
		return JsonResponse({"Error": "Method not allowed!"}, status=405)
...
def demandsAsRenting(request):
	if request.method == "GET":
		return APIrequests.GET(
			'article_demands',
			special_field={"id_author": request.user.pk, "visible": True, "status": "pending"})
	else:
		return JsonResponse({"Error": "Method not allowed!"}, status=405)
...
def currentTimelines(request):
	if request.method == "GET":
		docs = APIrequests.GET(
			'article_demands',
			special_field={"id_target": request.user.pk, "visible": True, "status": "accepted"},
			raw=True)
...
def currentTimelinesAsRenting(request):
	if request.method == "GET":
		docs = APIrequests.GET(
			'article_demands',
			special_field={"id_author": request.user.pk, "visible": True, "status": "accepted"},
			raw=True)
		for idx, document in enumerate(docs["article_demands"]):
			if (datetime.now(pytz.utc) > parse(document["availibility_end"])):
				db_locsapp["article_demands"].update({"_id": ObjectId(document['_id'])}, {
...
def verifyIfDemandAlreadyIssued(document):
	...
	elif (db_locsapp["article_demands"].find_one({"id_article": document["id_article"],
	                                              "id_target": document[
		                                              "id_author"]}) is not None):
...
def postNewMark(request):
	...
	if request.method == "POST":
		return APIrequests.POST(
			request, model, "articles", "The mark has been successfully added!",
			verifyIfNotAlreadyIssued)
	else:
		return JsonResponse({"Error": "Method not allowed!"}, status=405)
...
def verifyIfNotAlreadyIssued(document):
	...
	if ((document["as_renter"] and retrieve["id_author"] != document["id_author"]) or (
					document["as_renter"] is False and retrieve["id_target"] != document[
				"id_author"])):
	...
	if (db_locsapp["marks"].find_one({"id_demand": document["id_demand"], "id_article": document[
		'id_article'], "id_target": document["id_target"],
	                                  "id_author": document["id_author"]}) is not None):
